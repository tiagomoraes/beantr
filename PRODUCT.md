...
- The project's own prior landing pages: the warm-gradient, emoji-logo version, and the vellum-and-brass serif version that followed it. Stock coffee photography.
...
2. **Installation is the product surface.** A prompt the visitor pastes to their agent is the primary CTA, the copy-ready command is the secondary one; everything else supports getting installed.
...
5. **Composed, not loud.** One dark page, one crema accent, very large type and calm motion. Presence without gloss.

## Accessibility & Inclusion

Target WCAG 2.1 AA. Body text ≥ 4.5:1 and large/bold text ≥ 3:1, verified against the dark surface. Copy buttons and links are fully keyboard-operable with a visible crema focus ring. Honor `prefers-reduced-motion` (entrance reveals collapse to instant). Content must render fully without JavaScript and in headless/preview renderers — motion only ever enhances an already-visible default; it never gates visibility.
